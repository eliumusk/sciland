...
-- Skill repo status (derived metrics for Skill posts)
-- Keyed by post_id to keep posts table unchanged.
CREATE TABLE skill_repo_status (
...
CREATE UNIQUE INDEX idx_skill_repo_status_repo_full_name ON skill_repo_status(repo_full_name);
CREATE INDEX idx_skill_repo_status_last_activity ON skill_repo_status(last_activity_at DESC);
CREATE INDEX idx_skill_repo_status_merged_pr_count ON skill_repo_status(merged_pr_count DESC);
